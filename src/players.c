...
void player_check_board_collisions(uint8_t player_id) {
    ...
    uint8_t board_player_col = board_player_colors[player_id & 0x03u];
    ...
    if (p_player->speed_x != 0) {
        ...
        if (player_board_check_xy(test_x, PLAYER_TOP(py),    board_player_col)) p_player->bounce_x = true;
        if (player_board_check_xy(test_x, PLAYER_BOTTOM(py), board_player_col)) p_player->bounce_x = true;
    }
    ...
    if (p_player->speed_y != 0) {
        ...
        if (player_board_check_xy(PLAYER_LEFT(px),  test_y, board_player_col)) p_player->bounce_y = true;
        if (player_board_check_xy(PLAYER_RIGHT(px), test_y, board_player_col)) p_player->bounce_y = true;
    }
...
void players_reset(void) {
    ...
    for(uint8_t c = 0; c < gameinfo.player_count; c++) {
        if (c >= 4) {
            gameinfo.players[c] = gameinfo.players[(c & 0x03u) ^ 0x03u];
